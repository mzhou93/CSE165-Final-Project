...
bool Bullet::collision(){
    ...
    cout << AppWindow::instance()->user->x << endl;

    if (AppWindow::instance()->user->x  <= -.8) {
    ...
    if (AppWindow::instance()->user->x <= -.69 && AppWindow::instance()->user->x  >= -.8) { // change boundaries

        for (int i = 0; i < AppWindow::instance()->army1.size(); i++){ // change army

            float ex =  AppWindow::instance()->army1[lives1]->x;  // change lives and army.
            float ey =  AppWindow::instance()->army1[lives1]->y;  // change lives and army.

            AppWindow::instance()->windowToScene(ex, ey);

            if (_y <= ey+0.0002 && _y >= ey-0.05f && _x >= ex && _x <= ex+0.1f){ // bullet collision
                AppWindow::instance()->army1.erase(AppWindow::instance()->army1.begin()+lives1,AppWindow::instance()->army1.end()); // change lives and army
                AppWindow::instance()->user->deletebullet();
                lives1--; // change lives;
                return true;
            }
...
void Bullet::display(){	//draws bullet
    ...
    if (collision()){ // if collision happens pop the first bullet?
